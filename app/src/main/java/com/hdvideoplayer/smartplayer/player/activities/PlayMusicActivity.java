...
import com.hdvideoplayer.smartplayer.player.adapters.music.MusicArtPagerAdapter;
import com.hdvideoplayer.smartplayer.player.adapters.music.NextInMusicPlaylistAdapter.Callback;
...
import com.hdvideoplayer.smartplayer.player.dialog.ChangeVolumeBottomSheet;
import com.hdvideoplayer.smartplayer.player.dialog.MusicPlayerEqualizerDialogBuilder;
import com.hdvideoplayer.smartplayer.player.dialog.MusicPlayerNextInDialogBuilder;
import com.hdvideoplayer.smartplayer.player.services.MusicService;
import com.hdvideoplayer.smartplayer.player.services.MusicService.MyBinder;
import com.hdvideoplayer.smartplayer.player.services.MusicService.OnSongCallBack;
...
public class PlayMusicActivity extends BaseActivity implements OnClickListener, ServiceConnection, OnSongCallBack {
    ...
    private ImageView imgBtnNext, imgBack, imgBtnPrev, imgFavorite, imgEqualizer, imgMusicLyric, imgPlaylist, imgRepeat, imgShuffle, imgVolume;
    private int mPosition;
    private SeekBar seekBar;
    private TextView tvCurrentTime, tvSong, tvTitle, tvTotalTime, tvArtist;
...
//    }
    ...
    public void shareMusic(Context context, List<MusicInfo> musicInfo) {
        ...
        try {
            ...
            // Get the music file URI using FileProvider
            Uri musicUri = FileProvider.getUriForFile(context, context.getPackageName() + ".provider", new File(musicInfo.get(this.mPosition).getPath()));

            // Grant permission to read the URI
            intent.putExtra(Intent.EXTRA_STREAM, musicUri);
            intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION); // Ensure receiving app can read the file
            ...
            e.printStackTrace();
            FirebaseAnalyticsUtils.putEventClick(context, "Log_error", "share_music_error");
        }
    }
            ...
            if (!this.mSongList.isEmpty()) {
                ...
                MusicPlayerNextInDialogBuilder musicPlayerNextInDialogBuilder = new MusicPlayerNextInDialogBuilder(this, this.mSongList, this.mPosition, new Callback() {
                    public final void onMusicPlay(int i) {
                        PlayMusicActivity.this.onMusicPlayClick(i);
                    }
                });
    ...
    public void onMusicPlayClick(int i) {
        this.mPosition = i;
