...
import com.hdvideoplayer.smartplayer.player.adsprosimple.GlobalVar;
import com.hdvideoplayer.smartplayer.player.adsprosimple.OnActivityResultLauncher1;
import com.hdvideoplayer.smartplayer.player.adsprosimple.RemoteAppDataModel;
...
public class allInterstitial {
    ...
    public static void showInterstitial(Activity activity, OnActivityResultLauncher1.OnActivityResultLauncher2 resultLauncher) {
        ...
        if (GlobalVar.appData == null) {
            GlobalVar.appData = new RemoteAppDataModel();
            return;
        }

        if (GlobalVar.appData.getshowInterstitial() == null || GlobalVar.appData.getadstype() == null) {
            return;
        }
        ...
        if (GlobalVar.appData.getshowInterstitial() != 1) {
            doNext(activity, resultLauncher);
            return;
        }
        ...
        if (RemoteAppDataModel.adspersession == GlobalVar.appData.getAds_per_session()) {
            doNext(activity, resultLauncher);
            return;
        }

        if (GlobalVar.appData.getintertime() == 0) {
            interstitialcount++;
            if (interstitialcount % GlobalVar.appData.getinterclickcount() != 0) {
                doNext(activity, resultLauncher);
                return;
            }
        }

        switch (GlobalVar.appData.getadstype()) {
            case "facebook":
...
//                break;
    ...
    public static void fbinterstitial(Activity activity, OnActivityResultLauncher1.OnActivityResultLauncher2 resultLauncher) {
        String adUnitId = GlobalVar.appData.getfbInterstitialid();
...
//                    fbfullarraycount++;
                ...
                new CountDownTimer(GlobalVar.appData.getintertime() * 1000L, 1000) {
...
//            AdSettings.addTestDevice("HASHED ID");
    ...
    public static void aminterstitial(Activity activity, OnActivityResultLauncher1.OnActivityResultLauncher2 resultLauncher) {
        String adUnitId = GlobalVar.appData.getamInterstitialid();

        Log.e("PACK1_ADMOB_TAG", "Admob InterstitialAd ==>" + GlobalVar.appData.getamInterstitialid());
        if (TextUtils.isEmpty(adUnitId)) {
            return;
        }
        loadAdsDialog = new Dialog(activity);
    ...
    public static void adxinterstitial(Activity activity, OnActivityResultLauncher1.OnActivityResultLauncher2 resultLauncher) {
        String adUnitId = GlobalVar.appData.getAdxinterstitialid();

        Log.e("PACK1_ADMOB_TAG", "Adx InterstitialAd ==>" + GlobalVar.appData.getAdxinterstitialid());
        if (TextUtils.isEmpty(adUnitId)) {
            return;
        }
        loadAdsDialog = new Dialog(activity);
